...
public class ContinuousGWO_Schwefel {
        ...
        for (int t = 0; t < max_iter; t++) {
            ...
            for (int i = 0; i < n_agents; i++) {
                for (int j = 0; j < dim; j++) {
                    ...
                    double C1 = 2.0 * rand.nextDouble();
                    double C2 = 2.0 * rand.nextDouble();
                    double C3 = 2.0 * rand.nextDouble();

                    // Calculate A1, A2 and A3
                    double A1 = 2.0 * a * rand.nextDouble() - a;
                    double A2 = 2.0 * a * rand.nextDouble() - a;
                    double A3 = 2.0 * a * rand.nextDouble() - a;


                    //calculate new x for Alpha
                    double Da = Math.abs(C1 * Alpha_pos[j] - positions[i][j]);
                    double X1 = Alpha_pos[j] - A1 * Da;

                    //calculate new x for Beta
                    double Db = Math.abs(C2 * Beta_pos[j] - positions[i][j]);
                    double X2 = Beta_pos[j] - A2 * Db;

                    //calculate new x for Delta
                    double Dc = Math.abs(C3 * Delta_pos[j] - positions[i][j]);
                    double X3 = Delta_pos[j] - A3 * Dc;
    ...
    static void initializeAgents() {
        for (int i = 0; i < n_agents; i++) {
            for (int j = 0; j < dim; j++) {
                positions[i][j] = rand.nextDouble(lowerBound, upperBound);
            }
        }
    }
